...
export function getAdminApp(): App | null {
  ...
  if (typeof window !== "undefined") {
    console.warn(
      "❌ Firebase Admin SDK cannot be initialized in the browser environment"
    );
    return null;
  }
  ...
  if (!process.env.FIREBASE_SERVICE_ACCOUNT) {
    console.warn(
      "❌ FIREBASE_SERVICE_ACCOUNT environment variable is not set. Admin functions will not be available."
    );
    return null;
  }
      ...
      if (existingAdminApps.length > 0) {
        ...
        try {
          ...
          try {
            ...
            throw new Error(
              "FIREBASE_SERVICE_ACCOUNT must be a valid JSON string or base64-encoded JSON"
            );
          }
        }
