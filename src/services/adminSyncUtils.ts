...
export async function getUsersNeedingSync(): Promise<string[]> {
  try {
    ...
    const usersNeedingSync = userIds.filter(uid => !publicProfileIds.includes(uid));
    
    console.log(`📊 Found ${usersNeedingSync.length} users needing sync out of ${userIds.length} total users`);
...
export async function debugSyncStatus(): Promise<void> {
  try {
    ...
    if (usersNeedingSync.length > 0) {
      ...
      console.log("To fix, users need to log in again or admin can run bulk sync");
    } else {
    ...
    if (profilesWithoutUsernames.length > 0) {
      console.log(`⚠️ Found ${profilesWithoutUsernames.length} public profiles missing usernames:`, 
